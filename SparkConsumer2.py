...
output_path = "bowling_analysis_output"
if os.path.exists(output_path):
    shutil.rmtree(output_path)
os.makedirs(output_path)

spark = SparkSession.builder \
    .appName("IPL Bowling Analysis") \
    .getOrCreate()
...
def process_batch(df, epoch_id):
    ...
    if row_count > 0:
        ...
        print(f"Batch {epoch_id}: Bowling analysis processed and saved to {output_file}")
        df.show(truncate=False)
    else:
        print(f"Batch {epoch_id} had no data")

streaming_query = bowling_stats.writeStream \
    .outputMode("complete") \
